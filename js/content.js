			...
			if ($('#new_title').val().length > 0) {
				...
				success: function (result) {
					if(result.status) {
						...
						popupCloser($("#new_closeAddingSuccess"),$('#new_addingSuccess'),5000);
					} else {
						...
						console.log(e);
								...
								success: function (result) {
									if (result.status) {
										...
										popupCloser($("#show_closeUpdateSuccess"),$('#show_updateSuccess'),5000);
									} else {
										if (result.message=="InvalidSession") {
...
$(document).on('click', ".entry",function(event) {
		...
		success: function (result) {
			...
			event.stopImmediatePropagation();
			if(result.status) {
				var data = {id: result.id, title: result.title, user: result.user, password: result.password, urlentry: result.urlentry, note: result.note, iv: result.iv}; 
			...
			if ($('#profile_hint').val().length>0) {
				...
				success: function (result) {
					if (result.status) {
						...
						popupCloser($("#profile_closeUpdateSuccess"),$('#profile_updateSuccess'),5000);
					} else {
						if (result.message=="InvalidSession") {
	...
	$(document).on('click', '#settings_save', function() { // catch the form's submit event
			...
			success: function (result) {
				if (result.status) {
					...
					popupCloser($("#settings_closeUpdateSuccess"),$('#settings_updateSuccess'),5000);
				} else {
					if (result.message=="InvalidSession") {
									...
									$("#changepass_wait").on({
										popupafterclose: function(event, ui) {
											...
											if (cp_result !== false) {
												...
												popupCloser($("#changepass_closeUpdateSuccess"),$('#changepass_updateSuccess'),5000);
											} else {
												console.log(result.message);
...
$(document).on('click', "#show_copyuser",function(event) {
	...
	popupCloser($('#closePopupButton'),$("#show_copyPopup"),5000);
	$('#show_clearclipboard').fadeIn();
...
function popupCloser(closeButton,popupWindow,closeDelay) {
	var closeTxt = closeButton.html();
	var closesec = closeDelay;
	popupWindow.bind({
		popupafterclose: function(event, ui) {
			try {clearInterval(closePopupCounter);
			} catch (err) {}
			closeButton.html(closeTxt);
		}
	});
	try {clearInterval(closePopupCounter);
	} catch (err) {}
	closeButton.html(closeTxt+" ("+(closeDelay/1000)+")"); 
	closePopupCounter = setInterval(function(){ 
		...
		if (closesec<=0) {
			...
			closeButton.html(closeTxt+" ("+(closesec/1000)+")");
		}
	},1000);		
}
