...
$qry = "SELECT userid FROM ".$_SESSION['db_userTable']." WHERE username='".$username."'";
...
if ($username != null) {
	if ($res !== false) {
		$row  = $res->fetch_assoc();
		if ($row['userid'] == null || $row['userid'] == $_SESSION['uid']) {
		
			$pepper =bin2hex(generatePepper());
			$qry = "UPDATE ".$_SESSION['db_userTable']." SET password=AES_ENCRYPT('".$password."', SHA2('".$_REQUEST['pwd']."',512)), pepper=AES_ENCRYPT('".$pepper."', SHA2('".$_REQUEST['pwd']."',512)), username='".$username."' WHERE userid=".$_SESSION['uid'];
			$res = $mysqli->query($qry);
